...
def main():
    ...
    cell analysis, patch extraction, MIML inference, and cell sorting simulation.
    """
    ...
    # Set up cell sorting simulation
    output_dir = os.path.abspath("sorted_cells_output")
    cell_types = ['T4', 'T8']  # Based on your MIML model classes
    ...
    while True:
        ...
        for i, (track_id, (tx, ty)) in enumerate(tracked_results):
            ...
            if track_obj and (start_frame is not None) and (end_frame is not None) and not track_obj.done:
                ...
                if finalize_result is not None:
                    ...
                    if inference_out:
                        track_obj.done = True
                        # Process the sorted cell
                        sorter.process_cell(
                            patch=patch,
                            cell_type=inference_out['pred_class'],
        ...
        if DEBUG:
            ...
            # Draw rectangles around detected cells
            for i, ((tx, ty), box) in enumerate(zip(centers, boxes)):
                if box:
                    x, y, w, h = box
                    # Draw rectangle
                    cv2.rectangle(frame, (int(x), int(y)), (int(x + w), int(y + h)), (255, 0, 0), 2)
                    # Draw detection ID
                    cv2.putText(frame, f"Det:{i}", (int(x), int(y - 10)), 
